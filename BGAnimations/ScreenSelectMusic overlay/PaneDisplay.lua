...
local GetScoreFromProfile = function(profile, SongOrCourse, StepsOrTrail)
	...
	if not (profile and SongOrCourse and StepsOrTrail) then
		return nil
	end
...
local SetNameAndScore = function(name, score, nameActor, scoreActor, textColor)
	if not scoreActor or not nameActor then
		return
	end
...
for player in ivalues(PlayerNumber) do
	...
	af2.PlayerJoinedMessageCommand = function(self, params)
		if player == params.Player then
			...
			self:visible(true):zoom(0):croptop(0):bounceend(0.3):zoom(1):playcommand("Update")
		end
	end
	...
	af2.HideCommand = function(self)
		self:visible(false)
	end

	af2.OnCommand = function(self)
		self:playcommand("Set")
	end
	af2.SLGameModeChangedMessageCommand = function(self)
		self:playcommand("Set")
	end
	af2.CurrentCourseChangedMessageCommand = function(self)
		self:playcommand("Set")
	end
	af2.CurrentSongChangedMessageCommand = function(self)
		self:playcommand("Set")
	end
	af2["CurrentSteps" .. pn .. "ChangedMessageCommand"] = function(self)
		self:playcommand("Set")
	end
	af2["CurrentTrail" .. pn .. "ChangedMessageCommand"] = function(self)
		self:playcommand("Set")
	end
	...
	af2[#af2 + 1] = Def.Quad({
		...
		InitCommand = function(self)
			if player == PLAYER_1 then
				self:zoomtowidth(_screen.w / 2 + 11)
			elseif player == PLAYER_2 then
				self:zoomtowidth(_screen.w / 2 + 115)
			end
